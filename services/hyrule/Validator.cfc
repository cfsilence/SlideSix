			...
			if( !structKeyExists(prop,"fieldtype") || prop.fieldtype == "column"){
				...
				for(key in prop){
					...
					switch(key){
						...
						case "ALPHANUM" : {
							validator = new rules.AlphaNumValidator();
							break;
						}
	

						case "ISUNIQUEUSERNAME" : {
							validator = new rules.IsUniqueUserName();
							break;
						}
						...
						case "ISMATCH" : {
							...
							if( arrayLen(propertyMatch) ) {
								...
								for(var p=1; p<=arrayLen(props); p++){
									...
									if(props[p].name == property){
										prop.compareto = structKeyExists(props[p], "value") ? props[p].value : '';
									}
								}
							} else {
